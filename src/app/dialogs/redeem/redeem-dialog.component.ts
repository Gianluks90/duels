...
import type { RedeemCodeReward } from '../../models/redeem-code.model';
...
export class RedeemDialogComponent {
  ...
  protected readonly unlockedRewards = signal<RedeemCodeReward[] | null>(null);
  ...
  protected async redeem(): Promise<void> {
    ...
    this.error.set(null);
    try {
      const rewards = await this.auth.redeemCode(this.code());
      this.unlockedRewards.set(rewards);
    } catch (err) {
